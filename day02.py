...
spider_queue = MongoQueue('baiduSX','json_url')
...
def get_baidu(url,url_set):
    try:
        ...
        if len(response.text) > len('/**/jQuery110204907454377081921_1514968028687({"status":"0","t":"1514970458551","set_cache_time":"","data":[]});'):
            ...
            #保留一份json全的数据
            with open('json_data', 'a', encoding='utf-8') as f22:
                f22.write(str(result_list) + '\n')

            for each in result_list: #0的情况也能处理
                name = each['iname']
                cardNum = each['cardNum']
                with open('iname_cardNum_search_cardNum.txt','a', encoding='utf-8') as f:
                    f.write(name + ', ' + cardNum + '\n')
                print(name + ', ' + cardNum )

            print(len(result_list))
        else:
            with open('iname_cardNum_search_cardNum.txt', 'a') as f:
                f.write('\n')
